...
class Candidat extends DataBase
{
    ...
    public function addCandidat(string $lastName, string $firstName, string $description, string $birthDate, string $phone, string $mail, string $city, int $postalCode, string $adress, string $password, $id_profils, $id_contract, $id_domaine, $experienceYears, $cvPicture, $profilPicture): void
    {
        $db = $this->connectDb();
        $sql = "INSERT INTO `candidat` (`lastName`, `firstName`, `description`, `birthDate`, `phone`, `mail`, `city`, `postalCode`,`adress`, `password`, id_profils, id_contract, id_domaine, experienceYears, cvPicture, profilPicture) VALUES (:lastName, :firstName, :description, :birthDate, :phone, :mail, :city, :postalCode, :adress, :password, :id_profils, :id_contract, :id_domaine, :experienceYears, :cvPicture, :profilPicture)";
        $query = $db->prepare($sql);
        ...
        $query->bindValue(':experienceYears', $experienceYears, PDO::PARAM_INT);
